...
% - [ ] Have al figures named "Fig." and refer to them as such
% - [ ] abstract
% - [ ] Cite robolab (this is kinda mandatory, acknowledgements?)
% - [ ] Subtitle?
% - [ ] Add training parameters section (bezier, prepossecing, VAE architecture)
% - [ ] You could decide to say we trained our VAE for blah blah (in pytorch)
...
% (For example, species with distinct brain organisations but equivalent neuronal coding schemes for number exist, indicating that visual numerisoty discrimination is a evolutionary beneficial skill that forces neurons to automatically converge into a configuration that supports numerisoty perception as a perceptual category [@nieder2016neuronal]. ) (The existence of numerical abilities in innumerate cultures and human newbords further strengthens this point) Visual sense of number has been noted  to function independently from higher cogntive proceesses, such as. THis indepedence is assumed because .. Similairy, computiational models of numerical cognition provided only with sensory-like information (doing this and this) show that visual number sense is an emergent property of neurons (or neural populations) embedded in complex learning models that are not unlike the brain [for brains and hierachy, see zhou2006hierarchical, monkeys show hieracally organized brain regions performing in number processsing @nieder2016neuronal].

%% The experientially instantaneous capability to discriminate (judge?) visual numerisoties is an automatically developed property of neural populations found in many animals. The instantaneousness is product of the low-level (sometimes neural) cognitive facilitates, such as the neural populations that show tuning to a prefered numerosities in visual numerisoty estimation tasks.

% Mention the indepedence and invarience in related work? (yeah to further motivate a scale test)
%% 🐝 The experientially instantaneous capability to discriminate visual numerosities found in some animals is automatically developed at a neuronal level, and operates as an indepedent (indepedent whereof? it surely can't be 100\% independent) neuronal process by it omitting higher cogntive processes such as conscious representations \citep[p.~58 points to types of visual number sense being pre-attentive]{dehaene2011number} and symbolic processing [visual numerosity percepts are understood non-verbally, @nieder2016neuronal]. This indepedence is assumed because of the discovery of neural populations tuned to a preferred visual numerosity, whose response profiles remained invariant to all sensory modalities except quantity. The development of these neural populations numerosity performance being seperate from non-perceptual or symbolic knowledge of number is supported by the finding that these neural populations did not respond to their prefered numerosity when displayed in arabic numeral form [@harvey2013topographic], as well as the the existence of relatively uniformly functioning populations and related numerical abilities being present across different species, human newborns and innumerate cultures. For example, species with distinct brain organisations have shown the same neuronal coding for number, indicating that visual numerisoty discrimination is a evolutionary beneficial skill that forces neurons to automatically converge into a configuration that supports numerisoty perception as a perceptual category [@nieder2016neuronal]. '

% capability to perform numerisoties judgements...
% judge visual numerisoty
...
with discussing a dataset constructed for modeling subitizing in section \ref{related-work}, and then detail how we implemented our VAE algorithm to learn a representation of this dataset in \ref{vae}.  Slight problems during training and evaluation that arose from properties of our applied dataset and algorithm and appropriate solutions are examined in section \ref{hybrid}, \ref{deep-feature-consistent-perceptual-loss}} and \ref{imbalance}.  Next, as the subitizing task is essentially an image classification task, a full
methodology for evaluating the unsupervised VAE model's performance on
the subitizing classification task is described in section \ref{readout}. Subitizing performance is reported in section \ref{variational-autoencoder-performance}. Finally, section \ref{qualitative-analysis} highlights measurements of the final models robustness to changes in visual features.\\ \\
...
\begin{figure}
\centering
\includegraphics[height=6.2cm]{bezier.pdf}
...
\caption{Average Precision (\%) of various algorithms classifying subitize labels to images from the SOS test set. Accuracy scores of algorithms other than our \texttt{zclassifier} were copied over from \citet{zhang2016salient}. For their
...
%% Check if SIFT+IVF is completely non machine learning
...
%% Make every plot it's own figure (i.e. a-d?)
% Cumalative is still spelled wrong here
% export as pdf
\begin{figure*}[ht]
\centering
\begin{subfigure}{.5\textwidth}
  \centering
  \includegraphics[width=1.0\linewidth, height=10cm]{NN.pdf}
  \caption{}
  \label{fig:zn}
\end{subfigure}%
\begin{subfigure}{.5\textwidth}
  \centering
  \includegraphics[width=1.0\linewidth, height=10cm]{NA.pdf}
   \caption{}
  \label{fig:za}
\end{subfigure}
...
%%  Thanks to Robolab \citep{} for granting me access to their computing facilities. And special thanks to Tom for assisting me throughout this thesis, as well as helping me access the computer power that I need, (in the form of super computer cluster LISA). 
\newpage
\bibliography{thesis}
